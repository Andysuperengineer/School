...
sci_data<-read_csv("~/NIU/ETR537/Week3/Rtutorial/Data/sci-online-classes.csv")
```
...
-   Your response here; 30 observations.

Pick two columns (or more) and write what you think it represents:

-   Your response here; Student_id is the participant, course_id would be the subject the participant is learning, total_points_possible and 'earned' are the points the participant received and what would be the max amount of points they could have received if they got a perfect score.
...
-   Your response here; depends if you use \< or \> or the number, I used \>80 and I got less participants (or data).
...
sci_data|>
  select(course_id,TimeSpent_hours)|>
  skim(course_id)
```
...
ggplot(sci_data)+
  geom_histogram(aes(x=TimeSpent), fill="red")
...
```{r}
model_1<-lm(FinalGradeCEMS~TimeSpent+ Gender, data = sci_data)
summary(model_1)
```

What do you notice about the results? Add a comment or two below:

-   YOUR RESPONSE HERE; I added gender and it shows gender does not have the \*\*\* Which, if I am interpreting the data correctly means the gender, the smaller the p-value the less likely the results occurred by random chance, and the stronger the evidence that you should reject the null hypothesis. Since this is closer to 1 we should accept.
